...
const { registerPermissionSystemCommand } = await import(
  "../src/config-modal.js"
);
...
runTest(
  "permission-system command completions expose top-level config actions",
  () => {
...
await runAsyncTest(
  "permission-system command handlers manage config summary, persistence, and modal routing",
  async () => {
    const baseDir = mkdtempSync(
      join(tmpdir(), "pi-permission-system-command-"),
    );
    const configPath = join(baseDir, "config.json");
    let config: PermissionSystemExtensionConfig = {
      debugLog: true,
